...
import com.nd.hilauncherdev.framework.view.ISlidingConflict;
...
/**
 * 可以与ViewPager等滑屏组件嵌套使用的横向布局视图
 * 
 * @author Anson
 */
public abstract class NestedSlidingView extends ViewGroup implements OnClickListener, OnLongClickListener, ISlidingConflict {
	...
	public NestedSlidingView(Context context, AttributeSet attrs) {
		super(context, attrs);
		initWorkspace(context);
	}

	public NestedSlidingView(Context context, AttributeSet attrs, int defStyle) {
			...
			handler.postDelayed(new Runnable() {
				...
				public void run() {
					...
					if (lineLightbar != null) {
						lineLightbar.refresh(NestedSlidingView.this, mCurrentScreen * pageWidth);
					}
				}
			}, 100);
		}
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			if (isc != null) {
				isc.setChildSlidingViewState(true, isEndlessScrolling, getPageCount(), mCurrentScreen);
			}
	...
	public void setLineLightbar(NestedLineLightbar lineLightbar) {
